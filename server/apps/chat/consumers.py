...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    def saveMessage(self, message, userId, roomId, isRead):
    ...
    def updateChatRead(self, roomId):
        ChatMessage.objects.filter(chat__roomId=roomId).update(is_read=True)
    ...
    async def receive(self, text_data):
        ...
        print("RECEIVE JSON: ", text_data_json)
        action = text_data_json['action']
        roomId = text_data_json['roomId']
        isRead = text_data_json.get('isRead')
        chatMessage = {}
        if action == 'message':
            ...
            chatMessage = await database_sync_to_async(
                self.saveMessage
            )(message, userId, roomId, isRead)
        elif action == 'typing':
            chatMessage = text_data_json
        elif action == 'mark_as_read':
            # Update the chat room as read
            # and send back the payload
            await database_sync_to_async(self.updateChatRead)(roomId)
            chatMessage = text_data_json
